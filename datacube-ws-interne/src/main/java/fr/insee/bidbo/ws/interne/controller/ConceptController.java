...
public class ConceptController {
	...
	@POST
	@Path("/concept/chargement/fichier/valider")
	...
	public Response validerCodeListe(ValidationChoixGestionnaire validation) throws Exception {
		...
		chargementCodeListeService.chargerConcept(lignes, false);
		return Response.status(200).build();
	}

	@POST
	@Path("/concept/chargement/fichier/telecharger")
	@Consumes(MediaType.APPLICATION_JSON)
	...
	public Response telechargerCodeListe(ValidationChoixGestionnaire validation) throws Exception {
		...
		File fichier = FileUtils.getFile(validation.getLienFichier());
		List<String[]> lignes = CsvUtils.readAll(fichier, '#');
		List<Statement> triplets = chargementCodeListeService.chargerConcept(lignes, true);
		return ControllerUtils.lancerTelechargementTurtle(triplets, "concepts.ttle");
	}
